...
class FhevmDecryptionSignatureStorageKey {
  ...
  constructor(
    ...
    try {
      ...
      this.#key = `fhevm.decryptionSignature.${userAddress}:${hash}`;
    } catch (e) {
      console.log(e);
      throw e;
    }
...
export class FhevmDecryptionSignature {
  ...
  async saveToGenericStringStorage(
    ...
    try {
      ...
      console.log(
        `[FhevmDecryptionSignature] Signature saved! contracts=${this.#contractAddresses.length}`
      );
    } catch {
      console.error(
  ...
  static async loadFromGenericStringStorage(
    ...
    try {
      ...
      if (!result) {
        console.warn(`[FhevmDecryptionSignature] Could not load signature! key=${storageKey.key}`);
        return null;
      }
  ...
  static async new(
    ...
    try {
      ...
      console.error("[FhevmDecryptionSignature] Failed to create new signature:", error);
      return null;
    }
  }
